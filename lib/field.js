...
var Field = function ( prop, label, msg ){
  ...
  this.run = function ( src, form, opt ){
    ...
    function runStack( foo ){
      stack.forEach( function ( proc ){
        ...
        if( result.error ){
          ...
          result.error.msg = result.error.msg.replace( '%s', label );
          return errors.push( result.error );
        }
    ...
    // skip it if its not required & the client does not sent data
    // if( !self.__required && val === undefined ) return;
...
Field.prototype.custom = function ( func, msg ){
  ...
  return this.add( function ( val, src ){
    try{
      ...
      return { error : {
        field : self.name,
        msg   : msg || e.message || self.msg.custom
      }};
    }
