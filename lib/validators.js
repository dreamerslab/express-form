...
validators.isArray = function ( msg ){
  ...
  return this.add( function ( val ){
    ...
    return self.__checkIsArray ?
      { valid : true } :
      { error : msg };
  });
};

validators.isBoolean = validators.isBool = function ( msg ){
  msg = msg || this.msg.isBool;

  return this.add( function ( val ){
    // don't use `===` here
    // return utils.typeof( val ) === 'boolean' || val == 0 || val == 1 || val == true || val == false ?
    return [ 0, 1, '0', '1', true, false, 'true', 'false' ].indexOf( val ) > -1 ?
      { valid : true } :
      { error : msg };
  });
};
...
validators.isFloat = validators.isDecimal = function ( msg ){
  ...
  return this.add( function ( val ){
    val = utils.typeof( val ) === 'number' ? val.toString() : val;

    return utils.typeof( val ) === 'string' && val.match( /^[-+]?[0-9]*\.[0-9]+$/ ) ?
...
validators.regex = validators.is = function ( pattern, modifiers, msg ){
  ...
  return this.add( function ( val ){
    var err_msg = { error : msg || self.msg.regex };

    if( utils.typeof( val ) !== 'string' ) return err_msg;
